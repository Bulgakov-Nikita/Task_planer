...
//            $all_plans = tables\Plan::findAll(['sprav_kafedra_id' => $kafedra_id]);
            ...
            foreach ($path_dis as $pd){
                ...
                if (tables\Form::find()->andWhere(['type_form_id' => $type_form_id, 'disciplins_id' => $disciplins_id])->count() == 1) {
                    $table = tables\Form::findOne(['type_form_id' => $type_form_id, 'disciplins_id' => $disciplins_id]);
                    $table->updated_at = time();
                    $table->updated_by = Yii::$app->user->getId();
                }
                $table->data = $data;
    ...
    private function parse_kaf_has_comp_table() {
        ...
        foreach ($path_k as $pk) {
            ...
            foreach ($path_c as $pc) {
                ...
                if (tables\KafHasComp::find()->andWhere(['sprav_kafedra_id' => $kafedra_id, 'comp_id' => $comp_id])->count() == 1) {
                    $table = tables\KafHasComp::findOne(['sprav_kafedra_id' => $kafedra_id, 'comp_id' => $comp_id]);
                    $table->updated_at = time();
                    $table->updated_by = Yii::$app->user->getId();
                }
                $table->sprav_kafedra_id = $kafedra_id;
    ...
    private function parse_session_table(){
        $path_data = eval(Paths::$path_to_Планы);
        $path_ts = eval(Paths::$path_to_Заезды);
        $path_tw = eval(Paths::$path_to_СправочникВидыРабот);
        $path_plan = eval(Paths::$path_to_ООП);
        $path_k = eval(Paths::$path_to_ПланыГрафикиЯчейки);

        $data = (string) $path_data['ЧислоСессий'];

        foreach ($path_ts as $pts){
            $table = new tables\Session();
            $type_session = (string) $pts['Название'];
            $type_session_id = (tables\TypeSession::findOne(['name' => $type_session]))->id;
            foreach ($path_tw as $ptw){
                $type_work = (string) $ptw['Название'];
                $type_work_id = (tables\TypeWork::findOne(['name' => $type_work]))->id;
                foreach ($path_plan as $pp){
                    $kurs = (string) $path_k['Курс'];
                    $np_name = (string) $pp['Название'];
                    $np = tables\Np::findOne(['name' => $np_name]);
                    $np_id = $np->id;
                    $main_plan_id = (tables\MainPlan::findOne(['np_id' => $np_id]))->id;
                    $plan_id = (tables\Plan::findOne(['main_plan_id' => $main_plan_id, 'kurs_id' => $kurs]))->id;
                    if (tables\Session::find()->andWhere(['data' => $data, 'type_session_id' => $type_session_id, 'type_work_id' => $type_work_id, 'plan_id' => $plan_id])-> count() == 1){continue;}
