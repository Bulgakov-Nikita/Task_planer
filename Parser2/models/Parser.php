...
class Parser extends Model {
    ...
    private function parse_form_table() {
        ...
        $path_plans = eval(Paths::$path_to_Планы);
        $path_dis = eval(Paths::$path_to_ПланыЦиклы);
        ...
        foreach ($path_object_types as $type_object) {
            ...
            foreach ($path_dis as $pd){
                $disciplins = (string) $pd['Идентификатор'];
                $disciplins_id = (tables\Disciplins::findOne(['index' => $disciplins]))->id;

                if (tables\Form::find()->andWhere(['type_form_id' => $type_form_id, 'sprav_kafedra_id' => $kafedra_id, 'disciplins_id' => $disciplins_id])->count() == 1) {
                    continue;
                }
                $table->data = $data;
    ...
    private function parse_disciplins_table(){
        ...
        $index = $path_plans_cycle['Идентификатор'];
        foreach ($path_dis as $pd) {
        ...
        foreach ($path as $p) {
            ...
            foreach ($path1 as $p1){
                ...
                foreach ($path2 as $p2){
                    ...
                    $main_plan = (string) $p2['ГодНачалаПодготовки'];
                    $main_plan_id = (tables\MainPlan::findOne(['date_sp' => $main_plan]))->id;
            ...
            foreach ($path1 as $p1){
                ...
                foreach ($path2 as $p2){
                    ...
                    foreach ($path3 as $p3){
                        ...
                        $type_period_id = (tables\TypePeriod::findOne(['name' => $type_period]))->id;
            ...
            foreach ($path1 as $p1){
                ...
                foreach ($path2 as $p2){
                    ...
                    foreach ($path3 as $p3){
                        $table = new tables\Session();
                        $plan = (string) $p3[''];
                        $plan_id = (tables\Plan::findOne(['' => $plan]))->id;
...
//        $this->parse_type_form_table(); // 24
        $this->parse_form_table(); // 25 доделать disciplins_id
