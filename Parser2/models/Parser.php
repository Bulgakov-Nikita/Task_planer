...
class Parser extends Model {
    ...
    private function parse_form_table() {
        ...
        foreach ($path_object_types as $type_object) {
            ...
            foreach ($all_plans as $plan) {
                ...
                // нахождение атрибута disciplins_id
            }
        }
    }
    ...
    private function parse_disciplins_table(){
        ...
        $path_kaders = eval(Paths::$path_to_Кафедры);
        $path_plans = eval(Paths::$path_to_Планы);
        $path_sprav = eval(Paths::$path_to_ПланыСтроки);
        ...
        $sprav_dis = (string) $path_sprav['Дисциплниа'];
        $sprav_dis_id = (table\SpravDis::findOne(['name' => $sprav_dis]))->id;
        $table->index = $index;
        $table->sprav_kafedra_id = $kafedra_id;
        $table->sprav_dis = $sprav_dis;
        ...
        foreach ($path as $p) {
            ...
            foreach ($path1 as $p1){
                ...
                foreach ($path2 as $p2){
                    ...
                    $main_plan_id = (table\MainPlan::findOne(['date_sp' => $main_plan]))->id;
...
//      $this->parse_period_table(); // 18
 ...
 //       $this->parse_disciplins_table(); // 23 сделать
