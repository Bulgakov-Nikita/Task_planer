...
class Parser extends Model {
    ...
    private function parse_sprav_uch_god_table(){
        ...
        foreach ($path as $p) {
            ...
            foreach ($path1 as $p1){
                $table = new tables\SpravUchGod();
                $year_begin = (string) $p1['УчебныйГод'];
                if (tables\SpravUchGod::find()->andWhere(['year_begin' => $year_begin])->count() == 1){
                    continue;
                }
                $table->year_begin = $year_begin;
                foreach ($path1 as $p1){
                    $table = new tables\SpravUchGod();
                    $year_end = (string) $p1['УчебныйГод'];
                    if (tables\SpravUchGod::find()->andWhere(['year_end' => $year_end])->count() == 1){
                        continue;
                    }
                    $table->year_end = $year_end;
                    $table->create_at = time();
                    $table->create_by = Yii::$app->user->getId();
                    $table->update_at = time();
                    $table->update_by = Yii::$app->user->getId();
    ...
    public function parse_dc_table(){
        ...
        foreach ($path as $p) {
            ...
            foreach ($path1 as $p1){
                ...
                $comp = (string) $p1['ШифрКомпетенции'];
    ...
    private function comp_ps_has_comp_table(){
        $path = eval(Paths::$path_to_псГруппы);
        $path1 = eval(Paths::$path_to_ПланыКомпетенции);
        ...
        foreach ($path as $p) {
            ...
            $index = (string) $p['КодГруппы'];
            $comp_ps_id = (tables\CompPs::findOne(['index' => $index]))->id;
            $table->comp_ps_id = $comp_ps_id;
            foreach ($path1 as $p1){
                $table = new tables\CompPsHasComp();

                $index = (string) $p1['ШифрКомпетенции'];
                $comp_id = (tables\Comp::findOne(['index' => $index]))->id;
                if (tables\CompPsHasComp::find()->andWhere(['comp_ps_id' => $comp_ps_id, 'comp_id' => $comp_id])->count() == 1) {
                    continue;
                }
                $table->comp_id = $comp_id;
...
//        $this->parse_staff_table(); // Александр. Переделать, FIO теперь разделено на 3 атрибута
        ...
        // в процессе
//        $this->comp_ps_has_comp_table(); //Александр
