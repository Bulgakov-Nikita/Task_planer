...
class Parser extends Model {
    ...
    private function parse_podpisants_table(){
        ...
        foreach ($path as $p) {
            ...
            $staff = (string) $p['ФИО'];
            $staff_id = (tables\Staff::findOne(['' => $staff]))->id;
            foreach ($path1 as $p1){
                $table = new tables\Plan();

                $kurs = (string) $p1['Курс'];
                $kurs_id = (tables\Plan::findOne(['number_kurs' => $kurs]))->id;
...
//        $this->parse_form_table(); // 25 Добавить type_form_id(Готово) и plan_id(Под вопросом) + добавить апдейт(Готово)
