...
class Parser extends Model {
    ...
    private function parse_kaf_has_comp_table() {
        ...
        $path_comp_plans = eval(Paths::$path_to_ПланыКомпетенции);
        $path_plans = eval(Paths::$path_to_Планы);

        $kafedra_number = (string) $path_plans['КодПрофКафедры'];
        $kafedra_name = '';
        foreach ($path_kafedrs as $kafedra) {
            if (((string) $kafedra['Код']) == $kafedra_number) {
                $kafedra_name = (string) $kafedra['Название'];
                break;
            }
        }
        $sprav_kafedra_id = tables\SpravKafedra::findOne(['name' => $kafedra_name])->id;

        foreach ($path_comp_plans as $comp_plan) {
            $content = (string) $comp_plan['Наименование'];
            $comp_id = (tables\Comp::findOne(['soderzhanie' => $content]))->id;

            $table = new tables\KafHasComp();
            if (tables\KafHasComp::find()->andWhere([
                    'comp_id' => $comp_id,
                    'sprav_kafedra_id' => $sprav_kafedra_id
                ])->count() == 1){
                $table = tables\KafHasComp::findOne([
                    'comp_id' => $comp_id,
                    'sprav_kafedra_id' => $sprav_kafedra_id
                ]);
                $table->updated_at = time();
    ...
    private function parse_plan_table(){
        ...
        for ($kurs = 1; $kurs <= (integer) $number_courses; $kurs++) {
    ...
    private function parse_session_table() {
        $path_line_plans = eval(Paths::$path_to_ПланыСтроки);
        $path_sprav_type_works = eval(Paths::$path_to_СправочникВидыРабот);
        $path_sessions = eval(Paths::$path_to_ПланыНовыеЧасы);
        $path_sessionns = eval(Paths::$path_to_Заезды);

        foreach ($path_sessions as $session) {
            $data = (string) $session['Количество'];

            $kurs = (string) $session['Курс'];
            $kurs_id = (tables\Kurs::findOne(['number_kurs' => $kurs]))->id;

            $type_work_code = (string) $session['КодВидаРаботы'];
            $type_work_name = '';
            foreach ($path_sprav_type_works as $type_work) {
                if (((string) $type_work['Код']) == $type_work_code) {
                    $type_work_name = (string) $type_work['Название'];
                    break;
                }
            }
            $type_work_id = (tables\TypeWork::findOne(['name' => $type_work_name]))->id;

            $disciplins_code = (string) $session['КодОбъекта'];
            $disciplins_index = '';
            foreach ($path_line_plans as $line_plan) {
                if (((string) $line_plan['Код']) ==  $disciplins_code) {
                    $disciplins_index = (string) $line_plan['ДисциплинаКод'];
                    break;
                }
            }
            $disciplins_id = (tables\Disciplins::findOne(['index' => $disciplins_index]))->id;

            $session_number = (string) $session['Сессия'];
            $type_session_name = '';
            if ($session_number == '0') {
                $semestr_number = (string) $session['Семестр'];
                $type_session_name = 'Семестр ' . $semestr_number;
            } else {
                foreach ($path_sessionns as $session_type) {
                    if (((string) $session_type['Сессия']) == $session_number
                        and ((string) $session_type['Курс']) == $kurs) {
                        $type_session_name = (string) $session_type['Название'];
                        break;
                    }
                }
            }
            $type_session_id = (tables\TypeSession::findOne(['name' => $type_session_name]))->id;
    ...
    private function parse_dc_table(){
        $path_comp_plans = eval(Paths::$path_to_ПланыКомпетенции);
        $path_dis = eval(Paths::$path_to_ПланыСтроки);
        $path_dis_comp_plans = eval(Paths::$path_to_ПланыКомпетенцииДисциплины);

        foreach ($path_dis_comp_plans as $dis_comp_plan) {
            $dis_id = (string) $dis_comp_plan['КодСтроки'];
            $comp_plan_id = (string) $dis_comp_plan['КодКомпетенции'];

            $soderzhanie = '';
            foreach ($path_comp_plans as $comp_plan) {
                if (((string) $comp_plan['Код']) == $comp_plan_id) {
                    $soderzhanie = (string) $comp_plan['Наименование'];
                    break;
                }
            }
            $comp_id = (tables\Comp::findOne(['soderzhanie' => $soderzhanie]))->id;

            $disciplins_index = '';
            foreach ($path_dis as $discipline) {
                if (((string) $discipline['Код']) == $dis_id) {
                    $disciplins_index = (string) $discipline['ДисциплинаКод'];
                    break;
                }
            }
            $disciplins_id = (tables\Disciplins::findOne(['index' => $disciplins_index]))->id;
            ...
            $table = new tables\Dc();
            if (tables\Dc::find()->andWhere([
                    'comp_id' => $comp_id,
                    'disciplins_id' => $disciplins_id,
                ])->count() == 1) {
                $table = tables\Dc::findOne([
                    'comp_id' => $comp_id,
                    'disciplins_id' => $disciplins_id,
                ]);
                $table->updated_at = time();
