<?php

use \yii\db\Migration;

class m210521_194300_create_form_table extends Migration
{
    public function safeUp()
    {
        $this->createTable('form', [
            'id' => $this->primaryKey()->notNull()->comment('Первичный ключ'),
            'data' => $this->char(45)->notNull()->comment('данные'),
            'type_form_id' => $this->integer(11)->notNull()->comment('ссылка на тип формы'),
            'plan_id' => $this->integer(11)->notNull()->comment('ссылка на план'),
            'create_at' => $this->integer(11)->notNull()->comment('дата создания'),
            'create_by' => $this->integer(11)->notNull()->comment('кем создано'),
            'update_at' => $this->integer(11)->notNull()->comment('дата обновления'),
            'update_by' => $this->integer(11)->notNull()->comment('кем создано'),
            'delete_at' => $this->integer(11)->notNull()->comment('дата удаления'),
            'delete_by' => $this->integer(11)->notNull()->comment('кем удалено'),
            'active' => $this->tinyInteger(1)->notNull()->comment('статус'),
            'lock' => $this->integer(11)->notNull()->comment('блокировка')
        ]);
        $this->addCommentOnTable('form', 'Таблица для хранения информации о форме');

        $this->addForeignKey(
            'FK_type_form_id321',
            'form',
            'type_form_id',
            'type_form',
            'id');
        $this->addForeignKey(
            'FK_plan_id321',
            'form',
            'plan_id',
            'plan',
            'id');
    }

    public function safeDown()
    {
        $this->dropTable('form');
        $this->dropForeignKey('FK_type_form_id');
        $this->dropForeignKey('FK_plan_id');
    }
}
