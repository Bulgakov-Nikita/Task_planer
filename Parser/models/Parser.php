...
class Parser extends Model {
    ...
    private function parse_ze_table(){
        $path_dis = eval(Paths::$path_to_ПланыСтроки);

        foreach ($path_dis as $pd){
            $table = new tables\Ze;

            $dis = (string) $pd['ДисциплинаКод'];
            $disciplins_id = (tables\Disciplins::findOne(['index' => $dis]))->id;
            $ze = (string) $pd['ЗЕТфакт'];

            if (tables\Ze::find()->andWhere(['value' => $ze, 'disciplins_id' => $disciplins_id])->count() == 1) {continue;}
    ...
    private function parse_akadem_hours_table(){
        $path_d = eval(Paths::$path_to_ПланыСтроки);
        $path_ak_h = eval(Paths::$path_to_);

        foreach ($path_d as $pd){
            $dis = (string) $pd['ДисциплинаКод'];
            $disciplins_id = (tables\Disciplins::findOne(['index' => $dis]))->id;
            foreach ($path_ak_h as $pah){
                $hours = (string) $pah[''];
            }
        }
    }
...
//        $this->parse_ze_table(); // 32
        $this->parse_akadem_hours_table(); // 33
